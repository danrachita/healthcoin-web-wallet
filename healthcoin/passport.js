...
module.exports = function(passport) {
		...
		process.nextTick(function(){
			...
			User.findOne({'local.id': email}, function(err, user){
				...
				if(user){
					...
					if (new_address){
						user.wallet.push( { node_id: HCN.host, account: account, address: address });
					...
					newUser.wallet.push( { node_id: HCN.host, account: account, address: address });
	...
	passport.use('local-password', new LocalStrategy({
		...
		function(req, email, password, done){
			...
			if (typeof req.session.User.local === 'undefined'){
				return done(null, false, req.flash('passwordMessage', 'Please login first.'));
			}
			if (email !== req.session.User.local.id){
